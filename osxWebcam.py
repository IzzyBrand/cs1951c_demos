...
for i in range(5):
	a = aruco.drawMarker(aruco_dict, i, sidePixels = 1200, borderBits=1)
...
while True:
	...
	if rval:	# if we got a new frame
		corners, ids, rejected = aruco.detectMarkers(frame, aruco_dict)	# run aruco on the frame
		detected_frame = aruco.drawDetectedMarkers(frame, corners, ids)		# draw the boxes on the detected markers
		# print aruco.estimatePoseSingleMarkers(corners, markerEdgeLength, cameraMatrix, distCoeffs)
		rvecs, tvecs, extra = aruco.estimatePoseSingleMarkers(corners, markerEdgeLength, cameraMatrix, distCoeffs)

		# break
		if rvecs is not None:
			# print 'CORNERS', corners[0], corners[0].shape
			# print 'ROTATION', rvecs
			# print 'TRANSLATION', tvecs
			if np.array(rvecs).shape[0] > 1:
				rvecs = rvecs[0]
				tvecs = tvecs[0]

			# rot = cv2.Rodrigues(rvecs)	# get the rotation matrix
